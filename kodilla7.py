...
class visit_card:
    def __init__(self, imie, nazwisko, praca, firma, telefon):
        self.imie = imie
    ...
    def __str__(self):
        return f'{self.imie} {self.nazwisko} {self.praca},{self.firma}{self.telefon}'

def generuj_wizytowki(rodzaj=visit_card, ilosc=1):
    wizytowki = []
    if rodzaj == visit_card:
        for i in range(0, ilosc):
            kontakt  = visit_card( imie=fake.name(), nazwisko=(""), praca=fake.job(), firma=fake.company(), telefon=fake.phone_number())
            wizytowki.append(kontakt)

    return wizytowki

wizytowki = generuj_wizytowki(visit_card, 5)
for i in wizytowki:
    print(i)